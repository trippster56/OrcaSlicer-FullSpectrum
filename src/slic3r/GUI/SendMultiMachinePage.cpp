...
void SendDeviceItem::doRender(wxDC& dc)
{
    ...
    //device state
    if (state_printable <= 2) {
        dc.SetTextForeground(wxColour(18, 170, 224));
    }
    else {
    ...
    if (m_hover) {
        dc.SetPen(wxPen(wxColour(18, 170, 224)));
...
wxPanel* SendMultiMachinePage::create_page()
{
    ...
    // add send button
    btn_bg_enable = StateColor(std::pair<wxColour, int>(wxColour(14, 137, 179), StateColor::Pressed), std::pair<wxColour, int>(wxColour(61, 186, 230), StateColor::Hovered),
        std::pair<wxColour, int>(wxColour(18, 170, 224), StateColor::Normal));
